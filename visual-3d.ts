...
export class GdmLiveAudioVisuals3D extends LitElement {
  ...
  private init() {
    ...
    scene.add(sphere);
    sphere.visible = false; // Awalnya sphere tidak visible
    this.sphere = sphere;

    new EXRLoader().load(
      '/assets/piz_compressed.exr', // Ganti dengan path yang benar ke file EXR Anda
      (texture: THREE.Texture) => {
        texture.mapping = THREE.EquirectangularReflectionMapping;
        const exrCubeRenderTarget = pmremGenerator.fromEquirectangular(texture);
        sphereMaterial.envMap = exrCubeRenderTarget.texture;
        this.sphere.visible = true; // Jadikan sphere visible setelah tekstur dimuat
      },
    );
  ...
  private animation() {
    ...
    // Tambahkan pemeriksaan untuk memastikan analyser sudah ada
    if (!this.inputAnalyser || !this.outputAnalyser) {
      // console.warn('GdmLiveAudioVisuals3D: Analysers not yet initialized.');
      // Anda bisa memilih untuk merender scene kosong atau menunggu
      if (this.composer) this.composer.render(); // Render state saat ini (mungkin kosong)
      return;
    }
    ...
    // Pastikan sphere sudah ada sebelum mencoba mengakses materialnya
    if (!this.sphere) {
